...
import { addAcabamento, supabase } from "../../../../supabase";
...
export default function AddAcabamento({
  onClose,
  onProjectAdded,
}: AddProjectProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const mainImage of mainImages) {
      const { data, error } = await supabase.storage
        ...
        .upload(`acabamento-${mainImage.name}`, mainImage);
    ...
    for (const imageUrl of imageUrls) {
      const result = await addAcabamento({
        image_url: imageUrl,
      });
